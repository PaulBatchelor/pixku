...
runt_int runt_load_ll(runt_vm *vm, runt_ptr p);
...
static int rproc_loadpatchwerk(runt_vm *vm, runt_ptr p)
{
    runt_ptr patchwerk;
    runt_int rc;
    runt_stacklet *s;
    patchwerk = runt_load_patchwerk(vm);
    runt_mark_set(vm);
    rc = runt_ppush(vm, &s);
    RUNT_ERROR_CHECK(rc);
    s->p = patchwerk;
...
static int rproc_loadll(runt_vm *vm, runt_ptr p)
{
    runt_ptr patchwerk;
    runt_stacklet *s;
    runt_int rc;
...
int pixku_runt_loader(runt_vm *vm)
{
    ...
    runt_word_define(vm, "pix_ll", 6, rproc_loadll);
    pixku_circbuf(vm);
    pixku_pulse(vm);
    pixku_orbit(vm);
    return RUNT_OK;
